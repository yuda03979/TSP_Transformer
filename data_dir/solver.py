...
class Solver:
    ...
    def solve(self, board):
        if len(board) <= 8:
            ...
            return self.greedy_tsp(board)

    def solve_parallel(self, boards):
        """Solve multiple TSP instances in parallel using one algorithm."""
        s = time()
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            solutions = list(executor.map(self.solve, boards))
        if time() - s > 10:
            print(f'{time() - s=}')
        return solutions
